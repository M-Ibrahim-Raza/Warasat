...
const ParentsNodes = ({ parentsData }) => {
      ...
      svg.selectAll('.link')
        ...
        .attr('y1', d => {
          ...
          if (d.source.data.name === "Deceased") {
            return d.source.y + 430; // Standard offset for other nodes
          } else {
        ...
        .attr('y2', d => {
          // Apply offset based on the target node's name
          if (d.target.depth == 2) {
            return d.target.y - 260; // Standard offset for other nodes
          }
          return d.target.y + 120; // Standard offset for other nodes
        })
        .attr('stroke', '#ccc')
        .attr('stroke-width', 2);
