...
const Test3 = () => {
  ...
  useEffect(() => {
    ...
    const treeData = {
      ...
      children: [
        {
          name: "Father", share: 10, children: [
            { name: "Father", share: 30 },
    ...
    //   .attr('y1', d => d.source.y + 70) // Added an offset to prevent cutting off
    //   .attr('x2', d => d.target.x)
    //   // .attr('y2', d => d.target.y + 70) // Added an offset to prevent cutting off
    //   .attr('y2', d => d.data.name === "Wife" ? d.target.y + 70 : d.target.y + 70)
    //   .attr('stroke', '#ccc')
    //   .attr('stroke-width', 2);
    ...
    svg.selectAll('.link')
      ...
      .attr('y1', d => {
        ...
        if (d.source.data.name === "Deceased") {
          return d.source.y + 420; // Standard offset for other nodes
        } else {
          return d.source.y + 100; // Standard offset for other nodes
        }
      }
      )
      .attr('x2', d => d.target.x)
      .attr('y2', d => {
        // Apply offset based on the target node's name
        if (d.target.depth == 2) {
          return d.target.y - 170; // Standard offset for other nodes
        }
        return d.target.y + 150; // Standard offset for other nodes
      })
      .attr('stroke', '#ccc')
      .attr('stroke-width', 2);
    ...
    const nodes = svg.selectAll('.node')
      ...
      .attr('transform', d => {
        if (d.data.name === "Deceased") {
          return `translate(${d.x - 60},${d.y + 420})`
        } else if (d.depth === 2) {
          return `translate(${d.x - 60},${d.y - 220})`
        }
        else {
          return `translate(${d.x - 60},${d.y + 100})`
        }
      });
